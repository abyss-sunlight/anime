...
public class registerservlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if(password.equals("0")){//ÅÐ¶ÏÑéÖ¤Âë»¹ÊÇÃÜÂëµÇÂ¼
				//ÑéÖ¤ÂëµÇÂ¼
				List<jcxxbean> pd=ydo.yzmpd(mobile);
				if(pd==null){
					renderData(response, "ÊÖ»úºÅ´íÎó");
					return ;
				}
			...
			if(!jcbean.getCode().equals(verifyCode)){
				renderData(response, "ÑéÖ¤Âë´íÎó");
				return ;
			}
			if((System.currentTimeMillis() -jcbean.getCreateTime()) > 1000 * 60 * 10){
				renderData(response, "ÑéÖ¤ÂëÒÑ¹ýÆÚ");
				return ;
			}
			
			jcxxdao dao=new jcxxdao();
			int a=dao.login(jcbean,1);
			if(a==2){
				int yzmpd=ydo.yzmdate(jcbean);
				if(yzmpd==1){
					renderData(response, mobile);
					}else{
						renderData(response,"´úÂë´íÎó");
						return ;
					}
				}else{if(a==1){
					renderData(response,"ÊÖ»úºÅÎ´×¢²á" );
					return ;
				}else{
					renderData(response, "µÇÂ¼Ê§°Ü");
					return ;
				}
				}
			//ÃÜÂëµÇÂ¼
			}else{
			jcxxbean jcbean=new jcxxbean();
			jcbean.setSj(mobile);
			jcbean.setMm(password);
			jcxxdao dao=new jcxxdao();
			int a=dao.login(jcbean,2);
			if(a==2){		
					renderData(response, mobile);
				}else{if(a==1){
